...
from .models import OCRResult, SlideXMLStructure, VLMResult
...
1. The slide image
2. OCR-extracted text from the slide
3. An XML summary of the slide's structural elements, positions, and relationships
...
def run_vlm(
    ...
    Uses a cache keyed by (image hash, xml summary hash) to avoid redundant calls.
    """
    image_b64 = encode_image_base64(image_path)
    xml_summary = slide_to_xml_summary(xml_structure) if xml_structure else ""
    ocr_text = ocr_result.text if ocr_result else ""

    cache_key = f"vlm_{content_sha256(image_b64[:200], xml_summary[:200])[:16]}_slide{slide_num}"
    ...
    messages = build_vlm_messages(slide_num, image_b64, xml_summary, ocr_text)
    payload = {
        "model": cfg.vlm_model,
        "messages": messages,
